...
/* USER CODE BEGIN 4 */
const int MAX_LED = 4;             // Số lượng LED bảy đoạn
int index_led = 0;                 // Chỉ số LED hiện tại
int led_buffer[4] = {1, 2, 3, 4};  // Giá trị cần hiển thị trên LED bảy đoạn
...
int counter = 500;                  // Tăng giá trị này để giảm tốc độ nhấp nháy LED RED
int update_flag = 0;               // Biến cờ để điều khiển việc cập nhật hiển thị LED

// Hàm hiển thị số trên LED bảy đoạn
void display_number(int num) {
    // Tắt tất cả các đoạn LED
    ...
    switch (num) {
        ...
        case 4:
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET);  // SEG1
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2, GPIO_PIN_RESET);  // SEG2
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_RESET);  // SEG3
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);  // SEG4
            // SEG6 (SEG5 không bật)
            break;
        default:
            break;
...
// Hàm cập nhật hiển thị LED bảy đoạn
void update7SEG(int index) {
    ...
    switch (index) {
    case 0:
              display_number(led_buffer[0]); // Hiển thị giá trị từ led_buffer[0] trên EN0
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET); // Bật EN0
              break;
          case 1:
              display_number(led_buffer[1]); // Hiển thị giá trị từ led_buffer[1] trên EN1
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET); // Bật EN1
              break;
          case 2:
              display_number(led_buffer[2]); // Hiển thị giá trị từ led_buffer[2] trên EN2
              HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_RESET); // Bật EN2
              break;
          case 3:
              display_number(led_buffer[3]); // Hiển thị giá trị từ led_buffer[3] trên EN3
...
// Hàm được gọi khi ngắt timer xảy ra
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) {  // Nếu ngắt đến từ TIM2
        ...
        // Đặt cờ để cập nhật hiển thị LED bảy đoạn
        update_flag = 1;

        // Giảm biến counter và nhấp nháy LED D1 (PA5) mỗi khi counter về 0
        counter--;
        if (counter <= 0) {
            counter = 500;  // Tăng giá trị này để giảm tốc độ nhấp nháy LED RED
